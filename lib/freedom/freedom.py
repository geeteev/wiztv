...
addon = xbmcaddon.Addon('plugin.video.wiztv')
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FREEDOM IPTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(FREEDOM IPTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
                    ...
                    for i in sources:
                        try:
                            ...
                            if isinstance(i, list):
                                addDir(i[0].encode('utf-8'),i[1].encode('utf-8'),1901,icon,FANART,'','','','','source')
                            else:
                                thumb = icon
                                fanart = FANART
                                desc = ''
                                ...
                                if i.has_key('credits'):
                                    credits = i['credits']
                                addDir(i['title'].encode('utf-8'),i['url'].encode('utf-8'),1901,thumb,fanart,desc,genre,date,credits,'source')
                        except: traceback.print_exc()
                else:
                    if len(sources) == 1:
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(FREEDOM IPTV,New source added.,5000,"+icon+")")
        if not url is None:
        ...
        for i in soup('a'):
            ...
            if not href.startswith('?'):
                ...
                if name not in ['Parent Directory', 'recycle_bin/']:
                    if href.endswith('/'):
                        if browse:
                            addDir(name,url+href,1915,icon,fanart,'','','')
                        else:
                            addDir(name,url+href,1914,icon,fanart,'','','')
                    elif href.endswith('.xml'):
                        if browse:
                            addDir(name,url+href,1901,icon,fanart,'','','','','download')
                        else:
                            if os.path.exists(source_file)==True:
                                if name in SOURCES:
                                    addDir(name+' (in use)',url+href,11,icon,fanart,'','','','','download')
                                else:
                                    addDir(name,url+href,1911,icon,fanart,'','','','','download')
                            else:
                                addDir(name,url+href,1911,icon,fanart,'','','','','download')
...
def getCommunitySources(browse=False):
        ...
        for i in files:
            name = i('a')[0]['href']
            if browse:
                addDir(name,url+name,1901,icon,fanart,'','','','','download')
            else:
                addDir(name,url+name,1911,icon,fanart,'','','','','download')
    ...
    if isinstance(soup,BeautifulSOAP):
        if len(soup('channels')) > 0 and addon.getSetting('donotshowbychannels') == 'false':
            ...
            for channel in channels:
                ...
                try:
                    if linkedUrl=='':
                        addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),1902,thumbnail,fanArt,desc,genre,date,credits,True)
                    else:
                        addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1901,thumbnail,fanArt,desc,genre,date,None,'source')
                except:
...
def getChannelItems(name,url,fanart):
        ...
        for channel in channel_list('subchannel'):
            ...
            try:
                addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),1903,thumbnail,fanArt,desc,genre,credits,date)
            except:
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(FREEDOM IPTV,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def play_playlist(name, mu_playlist,queueVideo=None):
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            if index >= 0:
                if "&mode=1919" in mu_playlist[index]:
                    #playsetresolved (urlsolver(mu_playlist[index].replace('&mode=19','')),name,iconimage,True)
                    xbmc.Player().play(urlsolver(mu_playlist[index].replace('&mode=1919','').replace(';','')))
...
#                        print sepate
                        ...
                        url = urlsolver(i.replace('&mode=1919','').replace(';',''))                        
                    if url:
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('FREEDOM IPTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('FREEDOM IPTV', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            if len(parentalblockedpin)>0:
                if parentalblock:
                    contextMenu.append(('Disable Parental Block','XBMC.RunPlugin(%s?mode=1955&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
                else:
                    contextMenu.append(('Enable Parental Block','XBMC.RunPlugin(%s?mode=1956&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
                    
            if showcontext == 'source':
            
                if name in str(SOURCES):
                    contextMenu.append(('Remove from Sources','XBMC.RunPlugin(%s?mode=1908&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
                    
                    
            elif showcontext == 'download':
                contextMenu.append(('Download','XBMC.RunPlugin(%s?url=%s&mode=1909&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
            elif showcontext == 'fav':
                contextMenu.append(('Remove from wiztv Favorites','XBMC.RunPlugin(%s?mode=1906&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to wiztv Favorites','XBMC.RunPlugin(%s?mode=1905&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
    ...
    if json_folder_detail.has_key('error'):
        ...
        for i in json_folder_detail['result']['files'] :
            ...
            if i['filetype'] == 'file':
                ...
                addDir(name,url,1953,thumbnail,fanart,'','','','',allinfo=meta)
...
#                print 'setting as folder in link'
            contextMenu.append(('[COLOR white]!!Download Currently Playing!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1921&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        elif  (any(x in url for x in resolve_url) and  url.startswith('http')) or url.endswith('&mode=1919'):
            url=url.replace('&mode=1919','')
            mode = '1919'
            contextMenu.append(('[COLOR white]!!Download Currently Playing!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1921&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        elif url.endswith('&mode=1918'):
            url=url.replace('&mode=1918','')
            mode = '1918'
            contextMenu.append(('[COLOR white]!!Download!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1923&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
            if addon.getSetting('dlaudioonly') == 'true':
                contextMenu.append(('!!Download [COLOR seablue]Audio!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1924&name=%s)'
                                        %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        ...
        if 'plugin://plugin.video.youtube/play/?video_id=' in url:
              ...
              contextMenu.append(('!!Download [COLOR blue]Audio!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1924&name=%s)'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                u += "mode=1913&name=%s&playlist=%s" %(urllib.quote_plus(name), urllib.quote_plus(str(playlist).replace(',','||')))
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from wiztv Favorites','XBMC.RunPlugin(%s?mode=1906&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
            elif not name in FAV:
                try:
                    fav_params = (
                        '%s?mode=1905&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                ...
                contextMenu.append(('Add to wiztv Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        if not playlist is None:
            if addon.getSetting('add_playlist') == "false":
                playlist_name = name.split(') ')[1]
                contextMenu_ = [
                    ('Play '+playlist_name+' PlayList','XBMC.RunPlugin(%s?mode=19013&name=%s&playlist=%s)'
