...
ADDON_DATA = USERDATA_PATH + '/plugin.video.wiztv/'
...
def Menu(name, url, mode, iconimage, fanart, description, extra, showcontext=True, allinfo={}):
    ...
    if showcontext:
        ...
        if not name in FAV:
            contextMenu.append(('Add to wiztv Favorites',
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.wiztv-%s]: %s" % (addon_version, string))
...
def getFavourites():
    if os.path.exists(favourites) == False:
        favList = []
        addon_log('Making Favorites File')
        favList.append(('wiztv Favourites Section', '', '', '', '', '', ''))
        a = open(favourites, "w")
        a.write(json.dumps(favList))
        a.close()
    else:
