...
for these amazing sections all issues with errors in this addon wiztv should be reported to @Origin_Ent but streams not working taken to original 
author/playlister'''
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(wiztv ,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(wiztv ,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def SKindex_Silent():
    getData(base64.decodestring('aHR0cDovL3NpbGVudHN0cmVhbS5zcnZlLmlvL3NpbGVudGh1bnRlci9tYWluL2hvbWUueG1s'),'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from wiztv Favourites','XBMC.RunPlugin(%s?mode=1106&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to wiztv Favourites','XBMC.RunPlugin(%s?mode=1105&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from wiztv Favourites','XBMC.RunPlugin(%s?mode=1106&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to wiztv Favourites','XBMC.RunPlugin(%s)' %fav_params))
